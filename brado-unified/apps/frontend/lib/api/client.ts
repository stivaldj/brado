...
function debugLog(event: string, extra?: unknown): void {
  if (!IS_DEV) return;
  if (extra === undefined) {
    console.info(`[api] ${event}`);
    return;
  }
  console.info(`[api] ${event}`, extra);
}
...
async function request<T>(path: string, init?: RequestInit, retry = true): Promise<T> {
  ...
  if (response.status === 401 && retry && path !== `${API_PREFIX}/auth/token`) {
    debugLog("401 received, trying silent token refresh once", { path });
    await ensureToken();
  ...
  if (!response.ok) {
    ...
    debugLog("request failed", { path, status: response.status, message });
